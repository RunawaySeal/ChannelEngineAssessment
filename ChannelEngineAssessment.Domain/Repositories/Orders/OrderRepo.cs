...
{
  public class OrderRepo(HttpClient httpClient) : BaseRepo(httpClient), IOrderRepo
  {
    public async Task<ListResponse<Order>?> GetOrdersAsync(OrderFilters? filters)
    {
      // Build the URL with the filters
      var orderApiUrl = $"orders?apikey={ApiKey}";
      ...
      var orderJson = await _httpClient.GetStringAsync(orderApiUrl);
